...
<!-- <script type="text/javascript" src="newsList.js"></script>
 ...
 layui.use(['form','layer','laydate','table','laytpl'],function(){
	...
	            }else {
       ...
       $.ajax({
			type: "get",
			url: "${pageContext.request.contextPath}/CourseServlet/CourseList?t="+new Date().getTime(),
			dataType:"json",
			success:function(data){
	            //console.log(data);
	            datas=data.data;//注意此处
	...
	            }else {
	    ...
	    var tableIns = table.render({
	        elem: '#teacherList',
	        method: "post",
	        url : '${pageContext.request.contextPath}/SelectedCourseServlet/SelectedCourseList?t='+new Date().getTime(),
	        cellMinWidth : 95,
	        page : true,
	        height : "full-125",
	        limit : 10,
	   ...
	   /*  //是否置顶
	    ...
	    function addNews(edit){
	        var index = layui.layer.open({
	            title : "添加选课信息",
	            type : 2,
	            content : "${pageContext.request.contextPath}/SystemServlet/toSelectAdd",
	    ...
	    $(".delAll_btn").click(function(){
	        ...
	        if(data.length > 0) {
	            ...
	            layer.confirm('确定删除选中的选课信息？', {icon: 3, title: '提示信息'}, function (index) {
	               ...
	               /*  tableIns.reload();
	                ...
	                $.ajax({
						type: "post",
						url: "${pageContext.request.contextPath}/SelectedCourseServlet/DeleteSelectedCourse",
						data: {ids:ids},
	     ...
	     table.on('edit(teacherList)', function(obj){
			    ...
			    $.ajax({
					type: "post",
					url: "${pageContext.request.contextPath}/SelectedCourseServlet/EditSelectedCourse",
					data: data,
					success: function(msg){
						if(msg == "success"){
							tableIns.reload();
