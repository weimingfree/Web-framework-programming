...
<!-- <script type="text/javascript" src="newsList.js"></script>
 ...
 layui.use(['form','layer','laydate','table','laytpl'],function(){
       ...
       $.ajax({
			type: "get",
			url: "${pageContext.request.contextPath}/ClazzServlet/getClazzList?t="+new Date().getTime(),
			dataType:"json",
	...
	            }else {
	    ...
	    var tableIns = table.render({
	        elem: '#teacherList',
	        method: "post",
	        url : '${pageContext.request.contextPath}/TeacherServlet/TeacherList?t='+new Date().getTime(),
	        cellMinWidth : 95,
	        page : true,
	        height : "full-125",
	        limit : 10,
	    ...
	    function editNews(edit){
	        var index = layui.layer.open({
	            title : "编辑老师",
	            type : 2,
	            content : "${pageContext.request.contextPath}/SystemServlet/toTeacherEdit",
	    ...
	    function addNews(edit){
	        var index = layui.layer.open({
	            title : "添加老师",
	            type : 2,
	            content : "${pageContext.request.contextPath}/SystemServlet/toTeacherAdd",
	            success : function(layero, index){
	                var body = layui.layer.getChildFrame('body', index);
	                if(edit){
	                    body.find(".newsName").val(edit.name);
	        ...
	        if(data.length > 0) {
	            ...
	            layer.confirm('确定删除选中的教师？', {icon: 3, title: '提示信息'}, function (index) {
	               ...
	               /*  tableIns.reload();
	                ...
	                $.ajax({
						type: "post",
						url: "${pageContext.request.contextPath}/TeacherServlet/DeleteTeacher",
						data: {ids:ids},
